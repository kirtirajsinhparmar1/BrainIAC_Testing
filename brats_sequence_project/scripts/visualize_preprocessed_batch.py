"""Visualize one BraTSSequenceDataset batch after BrainIAC preprocessing."""
...
def _case_metadata(batch: dict[str, Any], index: int, volume: np.ndarray) -> dict[str, Any]:
    stats = volume_stats(volume)
    label = _label_to_int(_batch_value(batch, "label", index))
    return {
        "batch_index": index,
        "patient_id": str(_batch_value(batch, "patient_id", index)),
        "modality": str(_batch_value(batch, "modality", index)),
        "label": label,
        "label_name": label_name(label),
        "image_path": str(_batch_value(batch, "image_path", index)),
...
def _save_detailed_batch_figure(
    batch: dict[str, Any],
    ...
    fig, axes = plt.subplots(
        case_count,
        len(view_names),
        figsize=(12.5, max(2.1 * case_count, 4.0)),
        squeeze=False,
    )
    ...
    fig.suptitle(
        f"{split_name.title()} batch after {variant} preprocessing ({case_count} cases)",
        fontsize=15,
        y=0.995,
    )
...
def _save_axial_grid(
    ...
    for index, volume in enumerate(volumes):
        ...
        show_slice(
            axis,
            middle_slices(volume)["axial"],
            f"{row['patient_id']}\n{row['modality']} label {row['label_name']}",
        )

    fig.suptitle(f"{split_name.title()} axial middle slices after {variant} preprocessing", fontsize=15)
...
def main() -> None:
    ...
    metadata_rows = [_case_metadata(batch, index, volume) for index, volume in enumerate(volumes)]
    case_count = len(volumes)

    detailed_path = output_dir / f"batch_{args.split_name}_{args.variant}_{case_count}cases.png"
    metadata_path = output_dir / f"batch_{args.split_name}_{args.variant}_metadata.csv"
    stats_path = output_dir / f"batch_{args.split_name}_{args.variant}_stats.json"
    axial_grid_path = output_dir / f"batch_{args.split_name}_{args.variant}_axial_grid.png"

    _save_detailed_batch_figure(batch, volumes, metadata_rows, detailed_path, args.split_name, args.variant)
    _save_axial_grid(volumes, metadata_rows, axial_grid_path, args.split_name, args.variant)
